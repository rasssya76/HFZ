...
//limit2 FUNCTION
exports.giveTes = function(sender, duit){
  const _db = JSON.parse(fs.readFileSync('./database/user.json'))
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === sender) {
            position = i
    ...
    if (position !== false) {
        _db[position].tes += duit
        fs.writeFileSync('./database/user.json', JSON.stringify(_db))
    } 
}


exports.tesKurang = function(sender, duit){
  const _db = JSON.parse(fs.readFileSync('./database/user.json'))
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === sender) {
...
exports.getTes = function(sender){
  const _db = JSON.parse(fs.readFileSync('./database/user.json'))
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === sender) {
...
exports.addUserId = (gcount, limitCount, tanggal, pushname, sender, _level) => {                                                                                                                      
 obj = { name: pushname, id: sender, date: tanggal, xp: 1, level: 1, hit: 0, balance: 0, limit: limitCount, glimit: gcount, tes: 5 }
_level.push(obj)
fs.writeFileSync('./database/user.json', JSON.stringify(_level))
}
